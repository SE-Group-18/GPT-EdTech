...
const Summarizer = () => {
  ...
  const [mode, setMode] = useState<String>("bullet"); // State to toggle between paragraph and bullet points mode
    ...
    if (role === "summarizer") {
      ...
      try {
        const completion = await openai.chat.completions.create({
          messages: [
            { role: "system", content: "You are a React.js,Mongoose,Node.JS and JavaScript Expert. Give tsx or jsx code with explaination" }, // System message to define the AI's role
            { role: "user", content: inputText }, // User's input text with a prompt for summary
          ],
        ...
        if (completion.choices[0]?.message?.content) {
          ...
          const summarizedText: string =
            ...
            let textContent = summarizedText?.trim();
            let codeBlock = '';
            let language = '';
        ...
        setErrorMessage(
          `An error occurred while summarizing: ${err}`); // Set the error message
      }
    }
      ...
      >
        ...
        <div className="container mb-4" style={{ backgroundColor: "white" }}>
          ...
          <div className="d-flex flex-row">
            ...
            >
              <div style={{ minHeight: "40vh",maxWidth:"500px" }}>
